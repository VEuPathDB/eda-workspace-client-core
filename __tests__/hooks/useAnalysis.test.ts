...
import { useAnalysisState, Status } from '../../hooks/useAnalysis';
...
describe('useAnalysis', () => {
  ...
  it('should have the correct status on success path', async () => {
    const { result, waitForValueToChange } = renderHook(() => useAnalysisState(key, AnalysisApi, request));
  ...
  it('should load an analysis', async () => {
    const { result, waitFor } = renderHook(() => useAnalysisState(key, AnalysisApi, request));
  ...
  it('should update store on copy', async () => {
    ...
    expect(result.current.history.current).toEqual(newAnalysis);
